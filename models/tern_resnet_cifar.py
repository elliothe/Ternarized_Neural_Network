...
class ResNetBasicblock(nn.Module):
  ...
  def __init__(self, inplanes, planes, stride=1, downsample=None):
    ...
    self.conv_a = quanConv2d(inplanes, planes, kernel_size=3, stride=stride, padding=1, bias=False)
    self.bn_a = nn.BatchNorm2d(planes)

    self.conv_b = quanConv2d(planes, planes, kernel_size=3, stride=1, padding=1, bias=False)
